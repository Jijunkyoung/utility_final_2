...
(function (root, factory) {
  ...
  function fieldFor(requirement) {
    if (/선임|관리자/.test(requirement)) return 'legalMgr';
    if (/주기|정기검사|정기점검/.test(requirement)) return 'cycleMonths';
    if (/압력/.test(requirement)) return 'pressure';
    if (/용량|능력/.test(requirement)) return 'capacity';
    if (/냉동|냉난방/.test(requirement)) return 'hvac';
    return 'spec';
  }
  ...
  function law(equipment, documents) {
    ...
    (documents || []).forEach(function (doc) {
      ...
      obligations.forEach(function (requirement) {
        var field = fieldFor(requirement), actual = equipmentValue(equipment || {}, field);
        rows.push({ law: doc.law, requirement: requirement.slice(0, 300),
          equipmentField: actual.label, equipmentValue: actual.value || '미입력',
          status: actual.value ? '확인 필요' : '정보 부족',
          evidence: doc.effectiveDate ? '시행일 ' + doc.effectiveDate : (doc.sourceUrl || '내부 저장 자료'),
          action: actual.value ? '담당자가 법령 조건과 수치를 대조하세요.' : actual.label + '을(를) 입력하세요.' });
      });
    });
  ...
  function prompt(kind, equipment, textValue) {
    var schema = kind === 'manual'
      ...
      : '{"rows":[{"law":"","requirement":"","equipmentField":"","equipmentValue":"","status":"충족|미충족|확인 필요|정보 부족","evidence":"","action":""}],"warning":""}';
  ...
  return { manual: manual, law: law, prompt: prompt, normalizeAi: normalizeAi, cycleOf: cycleOf };
});
